...
public class NewCustomObjectActivityAsList1243 extends ParentActivity {
	
	@ObfuscateOPEN
	private static final String OBJ_POS = "objPos";
	//private static final String DEG_MIN_SEC = R.string.deg_min_sec;
...
//	private static final String SET_FIELD_VALUE = R.string.set_field_value_;
	...
	private static final String TAG="NCOA";@MarkTAG
	List<DepictableView> list;
	//Map<EditText,DepictableField> map=new HashMap<EditText,DepictableField>();//association of edit text with objects representing fields
	//Map<DepictableField,String> initialmap;//initial String values of fields
	DbListItem item;
	boolean editMode;
	CustomObject globj;
	//String initialObjString;
	...
	/*private Map<Integer,Integer> mapfi=new HashMap<Integer,Integer>();//association of field int with R.id. Like RA - R.id.nob_RA
	private void fillFieldIdMap(){
		mapfi.put(RA, R.id.nob_RA);
		mapfi.put(DEC, R.id.nob_DEC);
		//mapfi.put(TYPE, R.id.nob_TYPE_TYPESTR);
		mapfi.put(A, R.id.nob_A);
		mapfi.put(B, R.id.nob_B);
		mapfi.put(MAG, R.id.nob_MAG);
		mapfi.put(PA, R.id.nob_PA);
		mapfi.put(NAME1, R.id.nob_NAME1);
		mapfi.put(NAME2, R.id.nob_NAME2);
		mapfi.put(COMMENT, R.id.nob_COMMENT);
	}*/
	//int equinox=0;//2000 - 0; current - 1
	//int tmp_equinox=0;
	//int type=0;//obj type
	//int tmp_type=0;
	//int initial_type;
	//String initial_type_Str="";
	...
	Handler initHandler=new Handler(){
		...
		public void handleMessage(Message msg){
			...
			editMode=getIntent().getBooleanExtra(EDIT, false);//Settings.getSharedPreferences(this).getBoolean(Constants.NCOA_MODE, false);
			
			AstroObject aobj=Settings1243.getObjectFromSharedPreferencesNew(Constants.NCOA_OBJECT, NewCustomObjectActivityAsList1243.this);
			if(aobj instanceof CustomObject){
				globj=(CustomObject)aobj;
			}
			Log.d(TAG,""+getIntent().getExtras());
			//database we are working with
			int pos=getIntent().getIntExtra(CATALOG_POSITION, -1);//Settings.getSharedPreferences(this).getInt(Constants.NCOA_CAT_POS, -1);
...
//			Log.d(TAG,"iL="+iL);
			...
			if(editMode){
				...
				makeView(list);
				NewCustomObjectActivityAsList1243.this.setTitle(R.string.create_new_object);
			}

			//initialmap=getInitialValues(list);//initial object values
			...
			btnCancel.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View arg0) {				
					if(isDataChanged(list)){//data was changed 
						InputDialog d0 = new InputDialog(NewCustomObjectActivityAsList1243.this);
						...
						d0.setPositiveButton(getString(R.string.ok), new InputDialog.OnButtonListener() {
							public void onClick(String value) {
								NewCustomObjectActivityAsList1243.this.finish();
							}
						});
			...
			btnOK.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					if(!h.y&&!editMode){//new zero object
						registerDialog(InputDialog.message(NewCustomObjectActivityAsList1243.this,R.string.zero_object_will_not_be_added_to_the_database_)).show();
						return;
					}
					if(!h.y){//there were no changes
						Intent i=new Intent();
						i.putExtra(CHANGED, false);
						NewCustomObjectActivityAsList1243.this.setResult(Activity.RESULT_OK,i);
						finish();
						return;
					}
					Log.d(TAG,"error="+eh);
					...
					if(item.ftypes.isEmpty())
						catalog=new CustomDatabase(NewCustomObjectActivityAsList1243.this,item.dbFileName,item.cat);
					else
						catalog=new CustomDatabaseLarge(NewCustomObjectActivityAsList1243.this,item.dbFileName,item.cat,item.ftypes);
					...
					if (editMode){//existing object
						...
						if(obj.catalog!=AstroCatalog.CUSTOM_CATALOG){//updating obj right in the database
							...
							catalog.open(eh1);
							if(eh1.hasError())
								eh1.showError(NewCustomObjectActivityAsList1243.this);
							else{
								...
								catalog.close();
								ObsList1243.updateObsLists(obj,NewCustomObjectActivityAsList1243.this);//update observation lists if they contain the object
							}
						}
						else{//updating temp object right in the obs list
							int obsList=Settings1243.getSharedPreferences(NewCustomObjectActivityAsList1243.this).getInt(Constants.ACTIVE_OBS_LIST, InfoList.PrimaryObsList);
							//pos in global obs list
							int pos=NewCustomObjectActivityAsList1243.this.getIntent().getIntExtra(OBJ_POS, -1);
							if(pos==-1)return;
							...
							ObsList1243.copyObjectFields((CustomObject)item.x,obj);//item.x keeps a link to the object, so we need to change its fields
							new Prefs(NewCustomObjectActivityAsList1243.this).saveList(obsList);
						}
						finish();
					}
					else{//new object
						...
						if(obj.catalog!=AstroCatalog.CUSTOM_CATALOG){
							...
							catalog.open(eh2);
							if(eh2.hasError())
								eh2.showError(NewCustomObjectActivityAsList1243.this);
							else{
								catalog.add(obj,eh2);
								catalog.close();
							}
							...
							addObj(obj);
							//addObjToIntent(obj,i);//this object is read in calling obs list activity
						}
						finish();
					}
	 ...
	 * 
	 * @param item
	 * @return type of catalog objects or -1
	 */
	/*public static int catalogType(String dbname){
		//String dbname=item.dbName;
		if("WDS".equals(dbname))
			return AstroObject.DoubleStar;
		else if("DN(Barnard)".equals(dbname))
			return AstroObject.Neb;
		else if("UGC".equals(dbname))
			return AstroObject.Gxy;
		else if("BN(Lynds)".equals(dbname))
			return AstroObject.Neb;
		else if("DN(Lynds)".equals(dbname))
			return AstroObject.Neb;
		else return -1;
	}*/
	...
	private Holder2<CustomObject,Boolean> convertFieldsToObject(List<DepictableView> list,DbListItem item){
		...
		for(DepictableField f:dlist){
			...
			case EQ:
				...
				break;
			default:Object o=f.getInternalRep();
	...
	/*	if(type+1==AstroObject.Custom){
			typeStr=((EditText)findViewById(R.id.nob_TYPE_TYPESTR)).getText().toString();
		}*/
		CustomObject obj;
		if(fields.isEmpty())
	...
	private List<DepictableView> changeFieldsOrderForCMO(List<DepictableView> list){
		...
		/*List<DepictableView>list2=new ArrayList<NewCustomObjectActivityAsList1243.DepictableView>();
		for(DepictableView view:arr){
			list2.add(view);
		}*/
		...
		int i=0;
		for(DepictableView v:list){
			String fname=v.getField().getName();
			if(fname.equals(CMO.YEAR)){
				year=i;
			}
			else if(fname.equals(CMO.DAY)){
				day=i;
			}
			else if(fname.equals(CMO.MONTH)){
				month=i;
			}
			i++;
		}
		...
		if(day>0&&month>0&&year>0){
			List<DepictableView>list2=new ArrayList<DepictableView>();
			list2.add(list.get(year));
			list2.add(list.get(month));
			list2.add(list.get(day));
			i=0;
			for(DepictableView view:list){
				if(i!=year&&i!=day&&i!=month){
					list2.add(view);
				}
				i++;
			}
...
/*	private String stringValue(List<DepictableField> list){
		String s="";
		for(DepictableField d:list){
			s=s+d.getValue()+";";
		}
		return s;
	}*/
	
/*	private Map<DepictableField,String> getInitialValues(List<DepictableField> list){
		Map<DepictableField,String> map=new HashMap<DepictableField,String>();
		for(DepictableField f:list){
			map.put(f,f.getValue());
		}
		return map;
	}*/
				...
				tv2.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						...
						AstroTools.NameDialogRunnable r=new AstroTools.NameDialogRunnable(){
							...
							public void run(){
								...
								if(f.convert()){//successfull conversion
									...
									eh.showError(NewCustomObjectActivityAsList1243.this);
						...
						InputDialog d=AstroTools.getNameDialog
								(NewCustomObjectActivityAsList1243.this,title ,in_value,hint, phone_keyboard,r,majorTitle);
			...
			if(!block_listener){
				tv2.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {					
						...
						final InputDialog d1 = new InputDialog(NewCustomObjectActivityAsList1243.this);
						...
						d1.setListItems(R.array.object_types, new InputDialog.OnButtonListener() {
							...
							public void onClick(String value) {
								...
								if(oldSelection==row) d1.dismiss(); //no selection made, just close it
								...
								if(astro_type==AstroObject.Custom){
									...
									InputDialog d=AstroTools.getNameDialog(NewCustomObjectActivityAsList1243.this,
				...
				tv2.setOnClickListener(new View.OnClickListener() {
					...
					@Override
					public void onClick(View v) {	
						registerDialog(InputDialog.message(NewCustomObjectActivityAsList1243.this, 
				...
				tv2.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {	
						class Callback implements IPickFileCallback{
							DepictableField f;	
							TextView tv;
							public Callback(DepictableField f,TextView tv) {
						...
						FileDialogActivity1243.setListener(new Callback(f,tv2));
						Intent fileDialog = new Intent(NewCustomObjectActivityAsList1243.this, FileDialogActivity1243.class);
						startActivity(fileDialog);*/
						
						SelectFileActivity1243.setPath(Settings1243.getFileDialogPath(getApplicationContext()));
						SelectFileActivity1243.setListener(new Callback(f,tv2));
						Intent fileDialog = new Intent(NewCustomObjectActivityAsList1243.this, SelectFileActivity1243.class);
						startActivity(fileDialog);
					}
				});
	...
	class StringField implements DepictableField{
		...
		public String getType(){
			return DbListItem.FieldTypes.FIELD_TYPE_NAMES[1];
		}
	...
	class PhotoField extends StringField{
		...
		@Override
		public String getType(){
			return DbListItem.FieldTypes.FIELD_TYPE_NAMES[4];
		}
		@Override
		public Object getInternalRep(){
	...
	class UrlField extends StringField{
		...
		@Override
		public String getType(){
			return DbListItem.FieldTypes.FIELD_TYPE_NAMES[5];
		}
		@Override
		public Object getInternalRep(){
	...
	class IntField implements DepictableField{
		...
		public String getType(){
			return DbListItem.FieldTypes.FIELD_TYPE_NAMES[3];
		}
		public boolean convert(){
			try{
	...
	static class DoubleField implements DepictableField{
		...
		public String getType(){
			return DbListItem.FieldTypes.FIELD_TYPE_NAMES[2];
		}
		public boolean convert(){
			Double v;
			try{
